...
def MidTermSplit(source, datapath, outFolderName):
    ...
    for file in fileList:
        ...
        np.savez(outFolderName + '/' + ('_').join((user, exercise, repetition, 'NF')),
                 np.asarray(feature_vectors_nofatigue))
        np.savez(outFolderName + '/' + ('_').join((user, exercise, repetition, 'F')),
                 np.asarray(feature_vectors_fatigue))
        gt_labels[-1][-1] = 1
        np.savez('../original_labels_' + source + '/' + ('_').join((user, exercise, repetition)),
                 np.asarray(gt_labels[-1]))
